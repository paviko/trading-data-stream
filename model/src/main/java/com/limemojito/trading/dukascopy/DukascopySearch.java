...
import com.limemojito.trading.TickVisitor;
import com.limemojito.trading.TradingInputStream;
import com.limemojito.trading.TradingSearch;
import com.limemojito.trading.dukascopy.cache.LocalDukascopyCache;
...
public class DukascopySearch implements TradingSearch {
     ...
     *
     * @param args symbol, period, start (yyyy-MM-dd), end (yyyy-MM-dd)
     */
    @SuppressWarnings("MagicNumber")
    public static void main(String[] args) throws IOException {
        final Validator theValidator = setupValidator();
        DukascopySearch search = new DukascopySearch(theValidator, new LocalDukascopyCache(), new DukascopyPathGenerator());
        final String symbol = args[0];
        final Bar.Period period = Bar.Period.valueOf(args[1]);
        final LocalDate startDate = LocalDate.parse(args[2]);
        final LocalDate endTime = LocalDate.parse(args[3]);
        final int initialSize = 32 * 1024;
        final StringWriter stringWriter = new StringWriter(initialSize);
        try (TradingInputStream<Bar> ticks = search.search(symbol, period, startDate, endTime);
     ...
     */
    public TradingInputStream<Tick> search(String symbol, LocalDate startDate, LocalDate endTime, TickVisitor tickVisitor) {
    ...
    public TradingInputStream<Bar> search(String symbol,
        ...
        for (List<String> dayOfPaths : groupedPaths) {
            ...
            final TradingInputStream<Bar> oneDayOfBarStream = new DukascopyBarInputStream(validator, period, barVisitor, dayOfTicks);
